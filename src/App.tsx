...
import {
  ...
  Info, CheckCircle2, AlertCircle, Image as ImageIcon, FileDown, Layers, Eraser
} from 'lucide-react';
...
function Home() {
  useSeo({ title: 'PhotoTools - Free Online Photo Resize & Compress Tools', description: 'Free online photo tools that work entirely in your browser. Resize images, compress to exact file sizes, convert images to PDF, and remove backgrounds — no uploads, no sign-up.', path: '/' });
  const [openFaq, setOpenFaq] = useState<number | null>(0);
  const faqs = [
    ['Are my photos uploaded to a server?', 'Never. PhotoTools processes every image directly in your browser. Your files stay on your device from start to finish.'],
    ...
    ['Will compressing reduce my image quality?', 'Our smart compression finds the best balance between size and quality using an optimized quality search.'],
  ...
  return <>
    ...
    <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8"><AdPlaceholder variant="leaderboard" /></div>
    ...
    <section className="max-w-3xl mx-auto px-4 py-16"><div className="text-center mb-10"><p className="text-primary font-semibold text-sm uppercase tracking-wider mb-2">Good to know</p><h2 className="text-3xl sm:text-4xl font-bold text-app">Frequently asked questions</h2></div><div className="space-y-3">{faqs.map(([q, a], i) => <div key={q} className="card-surface overflow-hidden"><button onClick={() => setOpenFaq(openFaq === i ? null : i)} aria-expanded={openFaq === i} className="w-full flex items-center justify-between text-left p-5 font-semibold text-app"><span>{q}</span><ChevronDown className={`w-5 h-5 text-muted transition-transform ${openFaq === i ? 'rotate-180' : ''}`} /></button>{openFaq === i && <div className="px-5 pb-5 text-sm text-muted leading-relaxed animate-fade-in">{a}</div>}</div>)}</div></section>
  </>;
}
...
function ToolShell({ title, subtitle, icon, seoTitle, seoDescription, seoPath, children }: { title: string; subtitle: string; icon: ReactNode; seoTitle: string; seoDescription: string; seoPath: string; children: ReactNode }) {
...
function ResizeTool() {
  ...
  return <ToolShell title="Resize & Compress" subtitle="Resize by pixels, cm, or inches. Compress to an exact target file size." icon={<Maximize2 className="w-7 h-7" />} seoTitle="Photo Resizer - Resize Image to KB or MB Online | PhotoTools" seoDescription="Resize images by pixels, centimeters, or inches and compress to an exact file size in KB or MB. Free, private, and works entirely in your browser." seoPath="/resize">
    <AdPlaceholder variant="inline" className="mb-6" />
...
function SignatureTool() {
  ...
  return <ToolShell title="Signature Resizer" subtitle="Precise signature sizing with target file size limits for government and job forms." icon={<PenIcon />} seoTitle="Signature Resizer - Resize Signature Image Online | PhotoTools" seoDescription="Resize your signature image to exact pixel dimensions and file size limits for government forms, job applications, and passport documents. Free and private." seoPath="/signature">
...
function BackgroundTool() {
  ...
  const removeBackground = async () => {
    ...
    const onUnhandledRejection = (event: PromiseRejectionEvent) => {
      ...
      console.error('Unhandled rejection during background removal:', event.reason);
    };
    window.addEventListener('unhandledrejection', onUnhandledRejection);

    try {
      const input = await downscaleForBgRemoval(file, 1024);
      ...
      const result = await imglyRemoveBackground(input, {
        model: 'isnet_quint8',
        device: 'cpu',
      ...
      if (myJobId !== jobIdRef.current) return;
      const msg = err instanceof Error && (err.message.includes('fetch') || err.message.includes('network'))
  ...
  return <ToolShell title="Remove Background" subtitle="Remove backgrounds and apply custom colors or images. Your settings are saved for next time." icon={<Scissors className="w-7 h-7" />} seoTitle="Remove Image Background Online | PhotoTools" seoDescription="Remove the background from any photo using on-device AI. Replace it with a solid color, custom image, or keep it transparent. No uploads required." seoPath="/background">
...
function StaticPage({ type }: { type: 'about' | 'privacy' | 'terms' }) {
  const content = {
    about: { title: 'About PhotoTools', subtitle: 'Thoughtful tools for the way you work with images.', body: <><p>PhotoTools was created around a simple idea: everyday image tasks should feel effortless, not intimidating.</p><p>We make focused, beautifully designed utilities that do one thing well. Resize a photo for an application. Compress an image for faster sharing. Turn a set of memories into a PDF. Remove a background for a clean presentation.</p><p>What makes PhotoTools different is what we refuse to do. We do not require an account. We do not upload your files. We do not add watermarks. Your images are yours, and they stay on your device.</p></> },
    privacy: { title: 'Privacy Policy', subtitle: 'Your privacy is not a feature. It is the foundation.', body: <><h2>Local processing</h2><p>PhotoTools processes your images entirely within your web browser using HTML5 Canvas and the File API. Your files are never transmitted to, stored on, or accessible by our servers.</p><h2>Information we do not collect</h2><p>We do not collect your photos, image metadata, personal identity information, or the content of files processed through our tools. No account is required.</p><h2>Local storage</h2><p>The background removal tool saves your preferred background settings in your browser local storage. This data never leaves your device and can be cleared at any time.</p><h2>Advertising</h2><p>We may display advertisements to support the free service. Third-party advertising services, such as Google AdSense, may use cookies, device identifiers, or similar technologies to serve and measure ads based on your prior visits to this site or other sites. You can opt out of personalized advertising by visiting Google Ads Settings or your browser's privacy settings. These third parties operate under their own privacy policies, and PhotoTools does not control or access the data they collect.</p><h2>Contact</h2><p>If you have a privacy question, contact us at support@phototools.app.</p></> },
    terms: { title: 'Terms & Disclaimer', subtitle: 'Clear, fair, and straightforward.', body: <><h2>Use of the service</h2><p>PhotoTools is provided as a free utility for personal and commercial use. You are responsible for ensuring you have the right to process any images you use with the service.</p><h2>Accuracy and availability</h2><p>We work to keep our tools accurate and available, but provide them without warranties of any kind. Results may vary based on your browser, device, and source file.</p><h2>Limitation of liability</h2><p>To the fullest extent permitted by law, PhotoTools is not liable for any loss or damage arising from the use of these tools. Always keep an original copy of important files.</p><h2>Changes</h2><p>We may update these terms as the service evolves. Continued use of PhotoTools after changes means you accept the updated terms.</p></> },
  }[type];
  const seoConfig = {
    ...
    privacy: { title: 'Privacy Policy | PhotoTools', description: 'PhotoTools processes all images locally in your browser. We never upload, store, or access your files. Read our full privacy policy.', path: '/privacy' },
    terms: { title: 'Terms of Use | PhotoTools', description: 'PhotoTools is a free utility for personal and commercial use. Read our terms and disclaimer for full details on usage and limitations.', path: '/terms' },
  }[type];
...
function Contact() {
  ...
  useSeo({ title: 'Contact PhotoTools - Get in Touch', description: 'Have a question, idea, or need help with a PhotoTools feature? Send us a message and we will get back to you within one business day.', path: '/contact' });
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16"><div className="text-center mb-12"><div className="w-14 h-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mx-auto mb-5"><Mail /></div><h1 className="text-4xl font-bold text-app">We're here to help</h1><p className="text-muted mt-3">Have a question, idea, or just want to say hello?</p></div><div className="grid md:grid-cols-[.8fr_1.2fr] gap-6"><div className="card-surface p-6 h-fit space-y-5"><div><MessageCircle className="w-5 h-5 text-primary mb-3" /><h3 className="font-bold text-app">Support</h3><p className="text-sm text-muted mt-1">We typically respond within one business day.</p></div><div><Mail className="w-5 h-5 text-primary mb-3" /><h3 className="font-bold text-app">Email us</h3><p className="text-sm text-muted mt-1">support@phototools.app</p></div><div className="p-4 rounded-xl bg-primary/5 text-sm text-muted"><Shield className="w-4 h-4 text-accent inline mr-2" />No files are ever sent with your message.</div></div><div className="card-surface p-6 sm:p-8">{sent ? <div role="status" className="text-center py-10"><div className="w-14 h-14 rounded-full bg-accent/10 text-accent flex items-center justify-center mx-auto mb-4"><CheckCircle2 className="w-8 h-8" /></div><h2 className="text-xl font-bold text-app">Thanks for your message</h2><p className="text-muted mt-2">Please email us directly at support@phototools.app so we can respond. This form does not currently send messages.</p><button onClick={() => { setSent(false); setForm({ name: '', email: '', subject: '', message: '' }); }} className="btn-ghost text-primary mt-5">Send another message</button></div> : <form onSubmit={submit} className="space-y-4"><div className="grid sm:grid-cols-2 gap-4"><div><label htmlFor="contact-name" className="text-sm font-semibold text-app block mb-1.5">Your name</label><input id="contact-name" name="name" value={form.name} onChange={update} className="input-field" placeholder="Jane Smith" /></div><div><label htmlFor="contact-email" className="text-sm font-semibold text-app block mb-1.5">Email address</label><input id="contact-email" name="email" type="email" value={form.email} onChange={update} className="input-field" placeholder="jane@example.com" /></div></div><div><label htmlFor="contact-subject" className="text-sm font-semibold text-app block mb-1.5">Subject</label><input id="contact-subject" name="subject" value={form.subject} onChange={update} className="input-field" placeholder="How can we help?" /></div><div><label htmlFor="contact-message" className="text-sm font-semibold text-app block mb-1.5">Message</label><textarea id="contact-message" name="message" value={form.message} onChange={update} rows={5} className="input-field resize-none" placeholder="Tell us a little more..." /></div>{error && <p role="alert" className="text-sm text-rose-500 flex items-center gap-2"><AlertCircle className="w-4 h-4" />{error}</p>}<button className="btn-primary w-full justify-center"><Send className="w-4 h-4" />Send message</button></form>}</div></div></div>;
}

function NotFound() {
  useSeo({ title: 'Page Not Found | PhotoTools', description: 'The page you are looking for does not exist.', path: '/404' });
